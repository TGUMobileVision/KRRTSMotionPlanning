...
///得到轨迹的时长。
  ...
  void UniformBspline::setPhysicalLimits(const double &vel, const double &acc, const double &tolerance)
  {
      ...
      double dist =1;
      double max_r= max_vel_/(2*pow(max_acc_,2));
      double r_safe=0.5*0.5*(t_max-pow(2*dist/(max_acc_*0.707),0.5));
  ...
  bool UniformBspline::checkFeasibility(double &ratio, bool show)
  {
    ...
    for (int i = 0; i < P.cols() - 1; ++i)
    {
      ...
      if (fabs(vel(0)) > enlarged_vel_lim || fabs(vel(1)) > enlarged_vel_lim ||   ////绝对值不超过限制
          fabs(vel(2)) > enlarged_vel_lim)
      {
        ...
        for (int j = 0; j < dimension; ++j)
        {
          ...
          double r_safe=fabs(0.5*0.5*(t_max-pow(2*dist/(max_acc_*0.707),0.5)));
          double vv=max_vel+(r_safe/max_r)*(max_vel-vel(j));
          max_vel = max(vv, fabs(vel(j)));
        }
      }
    }
    ...
    for (int i = 0; i < P.cols() - 2; ++i)
    {
     // Ai = (p - 1) / (Ui+p+1 - Ui+2) * [ p / (Ui+p+2 - Ui+2) * (Qi+2 - Qi+1) - p / (Ui+p+1 - Ui+1) * (Qi+1 - Qi) ]
      ...
      if (fabs(acc(0)) > enlarged_acc_lim || fabs(acc(1)) > enlarged_acc_lim ||         ////绝对值不超过限制
