...
namespace ego_planner
{
  ...
  void BsplineOptimizer::setParam(ros::NodeHandle &nh)
  {
    ...
    nh.param("optimization/lambda_smooth_new", lambda1_new, -1.0);
    nh.param("optimization/lambda_collision_new", lambda2_new, -1.0);
    nh.param("optimization/lambda_feasibility_new", lambda3_new, -1.0);
    nh.param("optimization/lambda_fitness_new", lambda4_new, -1.0);

    nh.param("optimization/lambda_smooth_re", lambda1_re, -1.0);
    nh.param("optimization/lambda_collision_re", lambda2_re, -1.0);
    nh.param("optimization/lambda_feasibility_re", lambda3_re, -1.0);
    nh.param("optimization/lambda_fitness_re", lambda4_re, -1.0);
  ...
  void BsplineOptimizer::setEnvironment(const tgk_planner::PosChecker::Ptr &checker)
  {
    this->pos_checker_ = checker;
  }

  void BsplineOptimizer::setEnvironment(const tgk_planner::PosChecker::Ptr &checker, const fast_planner::ObjPredictor::Ptr mov_obj)
  {
    this->pos_checker_ = checker;
...
void BsplineOptimizer::setControlPoints(const Eigen::MatrixXd &points)
    ...
    for (int i = 0; i < seg_upbound; i++)
    {
      ...
      if (RichInfoSegs[i].first.size > 1)
      {
        ...
        for (int j = 0; j < RichInfoSegs[i].first.size - 1; j++)
        {
          ...
          for (double a = 1; a > 0; a -= step_size)
          {
            ...
            //cout << " " << grid_map_->validatePosSurround(pt) << " pt=" << pt.transpose() << endl;
            if (!pos_checker_->validatePosSurround(pt))
            {
              occ_start_id = j;
              occ_start_pt = pt;
      ...
      exit_multi_loop2:;
        ...
        for (int j = occ_start_id; j <= occ_end_id; j++)
        {
          ...
          if (!pos_checker_->validatePosSurround(base_pt_reverse)) // Search outward.
          {
            ...
            for (; l <= l_upbound; l += RESOLUTION)
            {
              ...
              if (pos_checker_->validatePosSurround(base_pt_temp))
              {
      ...
      else
      {
        ...
        if (!pos_checker_->validatePosSurround(base_pt_reverse)) // Search outward.
        {
          ...
          {
            Eigen::Vector3d base_pt_temp = base_pt_reverse + l * base_vec_reverse;
            //cout << base_pt_pos_checker_->validatePosSurroundtemp.transpose() << endl;
            if (pos_checker_->validatePosSurround(base_pt_temp))
            {
  ...
  std::vector<std::pair<int, int>> BsplineOptimizer::initControlPoints(Eigen::MatrixXd &init_points, bool flag_first_init /*= true*/)
  {
    ...
    for (int i = order_; i <= i_end; ++i)
    {
      ...
      {
        occ = !pos_checker_->validatePosSurround(a * init_points.col(i - 1) + (1 - a) * init_points.col(i));
        //cout << " " << occ;
        // cout << setprecision(5);
      ...
      for (int j = segment_ids[i].first + 1; j < segment_ids[i].second; ++j)
      {
        ...
        if (got_intersection_id >= 0)
        {
          ...
          if (length > 1e-5)
          {
            ...
            {
              occ =!pos_checker_->validatePosSurround((a / length) * intersection_point + (1 - a / length) * init_points.col(j));
  ...
  double BsplineOptimizer::costFunctionReboundReboundRebound(void *func_data, const double *x, double *grad, const int n)
  {
    BsplineOptimizer *opt = reinterpret_cast<BsplineOptimizer *>(func_data);

    double cost;
    opt->combineCostReboundReboundRebound(x, grad, cost, n);
  ...
  double BsplineOptimizer::costFunctionReboundRebound(void *func_data, const double *x, double *grad, const int n)
  {
    BsplineOptimizer *opt = reinterpret_cast<BsplineOptimizer *>(func_data);

    double cost;
    ///opt->old_combineCostReboundRebound(x, grad, cost, n);
    opt->combineCostReboundRebound(x, grad, cost, n);
    opt->iter_num_ += 1;
    //cout<<"cost+++++"<<cost<<endl;
    return cost;
  }
  ...
  void BsplineOptimizer::newyawcalcDistanceCostRebound(const Eigen::MatrixXd &q, double &cost,
                                                 Eigen::MatrixXd &gradient, int iter_num, double smoothness_cost)
  {
    ...
    if (iter_num > 3 && smoothness_cost / (cps_.size - 2 * order_) < 0.1) // 0.1 is an experimental value that indicates the trajectory is smooth enough.//0.1是一个实验值，表明轨迹足够平滑。
    {
 ...
 for (auto i = order_; i < end_idx; ++i)
  ...
  for (size_t j = 0; j < cps_.direction[i].size(); ++j)
    ...
    double max_r= (max_vel_*max_vel_)/(2*max_acc_);
    double t_max=1;//shi yan zhi
    ///double r=0;
  ///  for (int i = 0; i < q.cols() - 1; i++)
     ///{
      Eigen::Vector3d vi = (q.col(i + 1) - q.col(i)) / ts;
      double r=0.5*vi(1)*(t_max-pow(2*dist/(max_acc_*0.707),0.5));
      double flag_d_min=max_r/(2*max_vel_);
      double flag_d=r/((r/max_r)*vi(0)+vi(1));
      double dist_new=max_r-r;
      cout<< "rrrrrr.........."<<r<<endl;
     cout<< "maxr............."<<max_r<<endl;
      Eigen::Vector3d dist_grad = cps_.direction[i][j];
        ...
        cost += pow(dist_new, 3);
        gradient.col(i) += -3.0 * dist_new * dist_new * dist_grad;
  ...
  void BsplineOptimizer::newcalcDistanceCostRebound(const Eigen::MatrixXd &q, double &cost,
                                                 Eigen::MatrixXd &gradient, int iter_num, double smoothness_cost)
  {
    cost = 0.0;
    int end_idx = q.cols() - order_;
 ...
 for (auto i = order_; i < end_idx; ++i)
  ...
  for (size_t j = 0; j < cps_.direction[i].size(); ++j)
    ...
    double max_r= pow(max_vel_,2)/2*max_acc_;
    double t_max=1;//shi yan zhi
    ///double r=0;
    ///for (int i = 0; i < q.cols() - 1; i++)
  ///   {
      ...
      ///cout<<"vi......................"<<vi(0)<<endl;
      double r=0.5*vi(1)*(t_max-pow(2*dist/(max_acc_*0.707),0.5));
      double flag_d_max=max_r/(2*max_vel_);
      ///double flag_d=r/((r/max_r)*vi(0)+vi(1));
       double flag_d=r/((r/max_r)*max_acc_*0.707+max_acc_*0.707);
      double  dist_drr=fabs(max_r-r); 
      Eigen::Vector3d dist_grad = cps_.direction[i][j];

    
    if (flag_d>0&&flag_d<=0.25*flag_d_max)
    {
           
      cost += pow((0.5*vi(0)-dist_err),3);
       gradient.col(i) += -3.0 * ((0.5*vi(0)*0.5-dist_err)*(0.5*vi(0)-dist_err)*dist_grad);  
 
    } 
    
     else if (flag_d>0.75*flag_d_max&&flag_d<=flag_d_max)
      {

          cost += 0.7*pow(dist_err, 3)+0.3*(pow((vi(0)*0.5-dist_err),3));
        gradient.col(i) += -3.0 * (dist_err * dist_err * dist_grad + (vi(0)*0.5-dist_err)*(0.5*vi(0)-dist_err)*dist_grad);
  ...
  void BsplineOptimizer::newcalcSmoothnessCost(const Eigen::MatrixXd &q, double &cost,
  ...
  void BsplineOptimizer::newvcalcFeasibilityCost(const Eigen::MatrixXd &q, double &cost,
    ...
    for (int i = 0; i < q.cols() - 1; i++)
    {
      ...
      for (int j = 0; j < 3; j++)
      {
        if (vi(j) > max_vel_ + demarcation)
        {
          ...
          cost += (ar * diff * diff + br * diff + cr) * ts_inv3; // 乘以ts_inv3，使vel和acc具有相似的大小 multiply ts_inv3 to make vel and acc has similar magnitude
...
void BsplineOptimizer::newacalcFeasibilityCost(const Eigen::MatrixXd &q, double &cost,
                                             Eigen::MatrixXd &gradient)
  {
  ...
  bool BsplineOptimizer::check_collision_and_rebound(void)
  {
    ...
    ///int i_end = end_idx - 2*(end_idx - order_) / 3;
    for (int i = order_ - 1; i <= i_end; ++i)
    {
      

      ///cout<<"cps_.points.col(i)++++++++++"<<cps_.points.col(i)<<endl;
      bool occ = !pos_checker_->validatePosSurround(cps_.points.col(i));
      ...
      if (occ)
      {
        ...
        for (j = i - 1; j >= 0; --j)
        {
          occ = !pos_checker_->validatePosSurround(cps_.points.col(j));
           cout<<"occ2++++++++++"<<occ<<endl;
          if (!occ)
          {
            in_id = j;
        ...
        for (int j = segment_ids[i].first + 1; j < segment_ids[i].second; ++j)
        {
          ...
          if (got_intersection_id >= 0)
          {
            ...
            if (length > 1e-5)
            {
              ...
              for (double a = length; a >= 0.0; a -= pos_checker_->getResolution())
              {
                bool occ = !pos_checker_->validatePosSurround((a / length) * intersection_point + (1 - a / length) * cps_.points.col(j));
 ...
 Eigen::MatrixXd  BsplineOptimizer::BsplineOptimizeTrajReboundRebound(Eigen::MatrixXd &optimal_points, double ts)
 {

   setBsplineInterval(ts);

    double final_cost;
    bool flag_success = reboundrebound_optimize(final_cost);
  ...
  ///bool flag_success = reboundreboundrebound_optimize(final_cost);
  bool flag_success = rebound_optimize(final_cost);
    optimal_points = cps_.points;
...
Eigen::MatrixXd BsplineOptimizer::BsplineOptimizeTrajRefineRefine(const Eigen::MatrixXd &init_points, const double ts, Eigen::MatrixXd &optimal_points)
  {
...
//*****************************************************************************************************
  bool BsplineOptimizer::reboundreboundrebound_optimize(double &final_cost)
  {
    ...
    do
    {
          ...
          result == lbfgs::LBFGS_STOP)
      {
        ...
        {
          flag_occ = !pos_checker_->validatePosSurround(traj.evaluateDeBoorT(t));
          if (flag_occ)
          {
...
//*****************************************************************************************************************
  bool BsplineOptimizer::reboundrebound_optimize(double &final_cost)
  {
    ...
    do
    {
      ...
      /* ---------- optimize ---------- */
      t1 = ros::Time::now();
      int result = lbfgs::lbfgs_optimize(variable_num_, q, &final_cost, BsplineOptimizer::costFunctionReboundRebound, NULL, BsplineOptimizer::earlyExit, this, &lbfgs_params);
      //cout<<"variable_num_++++++"<<variable_num_<<endl;
      //cout<<"&final_cost+++++++++++"<<&final_cost<<endl;
      t2 = ros::Time::now();
      double time_ms = (t2 - t1).toSec() * 1000;
...
//#define USE_SECOND_CLEARENCE_CHECK
  ...
  bool BsplineOptimizer::rebound_optimize(double &final_cost)
  {
    ...
    do
    {
      ...
      /* ---------- optimize ---------- */
      t1 = ros::Time::now();
      int result = lbfgs::lbfgs_optimize(variable_num_, q, &final_cost, BsplineOptimizer::costFunctionReboundReboundRebound, NULL, BsplineOptimizer::earlyExit, this, &lbfgs_params);
      //cout<<"variable_num_++++++"<<variable_num_<<endl;
      //cout<<"&final_cost+++++++++++"<<&final_cost<<endl;
      t2 = ros::Time::now();
      double time_ms = (t2 - t1).toSec() * 1000;
...
//#define USE_SECOND_CLEARENCE_CHECK
  ...
  bool BsplineOptimizer::refine_optimize()
  {
    ...
    do
    {
      ...
      {
        if (!pos_checker_->validatePosSurround(traj.evaluateDeBoorT(t)))
        {
  ...
  void BsplineOptimizer::old_combineCostReboundRebound(const double *x, double *grad, double &f_combine, const int n)
  {
   ...
   // cout << "n=" << n << endl;
    ...
    f_combine = lambda1_ * f_smoothness + lambda3_ * f_feasibility;
   ...
   //cout<<"f_combine"<<f_combine<<endl;
         Eigen::MatrixXd grad_3D = lambda1_ * g_smoothness + lambda3_ * g_feasibility;
    //Eigen::MatrixXd grad_3D = lambda1_ * g_smoothness + new_lambda2_ * g_distance + lambda3_ * g_feasibility + new_lambda2_ * g_mov_objs;
     //cout<<"grad_3D"<<grad_3D<<endl;
    memcpy(grad, grad_3D.data() + 3 * order_, n * sizeof(grad[0]));
  }


  void BsplineOptimizer::combineCostReboundRebound(const double *x, double *grad, double &f_combine, const int n)
  {
    ...
    calcSmoothnessCost(cps_.points, f_smoothness, g_smoothness);
    newyawcalcDistanceCostRebound(cps_.points, f_distance, g_distance, iter_num_, f_smoothness);
    newvcalcFeasibilityCost(cps_.points, f_feasibility, g_feasibility);
 
    calcSwarmCost(cps_.points, f_swarm, g_swarm);
    calcTerminalCost( cps_.points, f_terminal, g_terminal );

    f_combine =0.5*(0.4*new_lambda2_ * f_distance+lambda3_ * f_feasibility); 

    Eigen::MatrixXd grad_3D = 0.5*(0.4*new_lambda2_ * g_distance+lambda3_ * g_feasibility);

    memcpy(grad, grad_3D.data() + 3 * order_, n * sizeof(grad[0]));
  }

  void BsplineOptimizer::combineCostReboundReboundRebound(const double *x, double *grad, double &f_combine, const int n)
  {
    ...
    calcSmoothnessCost(cps_.points, f_smoothness, g_smoothness);
    newcalcDistanceCostRebound(cps_.points, f_distance, g_distance, iter_num_, f_smoothness);
    calcFeasibilityCost(cps_.points, f_feasibility, g_feasibility);

    calcTerminalCost( cps_.points, f_terminal, g_terminal );
     f_combine = lambda1_new* f_smoothness + new_new_lambda2_ * f_distance + lambda3_new * f_feasibility + lambda2_new * f_terminal ;

   Eigen::MatrixXd grad_3D =lambda1_new* g_smoothness + new_new_lambda2_ * g_distance + lambda3_new * g_feasibility + lambda2_new * g_terminal;
  ...
  void BsplineOptimizer::combineCostRefine(const double *x, double *grad, double &f_combine, const int n)
  {
    ...
    f_combine = lambda1_re* f_smoothness + lambda4_re * f_fitness + lambda3_re * f_feasibility;


    Eigen::MatrixXd grad_3D = lambda1_re* g_smoothness + lambda4_re * g_fitness + lambda3_re * g_feasibility;
    memcpy(grad, grad_3D.data() + 3 * order_, n * sizeof(grad[0]));
  }
