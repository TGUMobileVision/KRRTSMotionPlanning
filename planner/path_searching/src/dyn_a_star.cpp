...
void AStar::initGridMap(const tgk_planner::PosChecker::Ptr &pos_checker, const Eigen::Vector3i pool_size)
{
            ...
            for (int dy = -1; dy <= 1; dy++)
                for (int dz = -1; dz <= 1; dz++)
                {
                    ...
                    if (checkOccupancy(Index2Coord(neighborPtr->index)))
                    {
                     ...
                     double h=1;
                    double static_cost = sqrt(dx * dx + dy * dy + dz * dz)+0.5*0.5*(h/dz+dz/h);
