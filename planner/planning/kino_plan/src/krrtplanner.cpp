...
void KRRTPlanner::init(const ros::NodeHandle& nh)
{
  ...
  ROS_WARN_STREAM("[krrt] param: : " << radius_cost_between_two_states_);
...
void KRRTPlanner::setPosChecker(const tgk_planner::PosChecker::Ptr &checker)
{
                      ...
                      double search_time, const Vector3d &normal, const Vector3d &dire, bool need_consistancy)
{
  ...
  reset();
  ROS_ERROR_STREAM("krrt start pos: " << start_pos.transpose());
  if (!pos_checker_ptr_->validatePosSurround(start_pos)) 
  {
    ROS_ERROR("[KRRT]: Start pos collide or out of bound");
    return FAILURE;
  }
  ROS_WARN_STREAM("krrt end pos: " << end_pos.transpose());
...
{ 
  ...
  /* local variables */
  /*局部变量*/
  int valid_sample_nums = 0; //random samples in obs free area //在obs free区随机采样
  ...
  for (idx = 0; (ros::Time::now() - rrt_start_time).toSec() < search_time && valid_start_tree_node_nums_ < n ; ++idx) 
  {
   ...
   valid_samples.push_back(x_rand);
    ...
    while(!kd_res_end(p_bcwd_nbr_set)) 
    {
      ...
      kd_res_next(p_bcwd_nbr_set); //go to next in kd tree range query result ////转到下一个kd树范围查询结果
    }
    kd_res_free(p_bcwd_nbr_set); //reset kd tree range query   //重置kd树范围查询
      ...
      if (connected_to_goal) 
      {
        ...
        if (first_time_find_goal) 
        {
          ...
          while (node->parent) 
          {
            ...
            //新建一个RRTNodePtr而不是在start_tree_中使用ptr，因为随着采样和重新连接的发生，
          // start_tree_[i]的parent, children和其他属性可能会改变。
        ...
        if (stop_after_first_traj_found_) break; //stop searching after first time find the goal?        ////停止搜索后，第一次找到目标?
      }
